...
import { generateSparqlQuery, fetchSparqlResults } from './requete'; // Import des fonctions pour les requêtes SPARQL
import Link from 'next/link'; // Import de Link pour créer des liens dynamiques
...
export default function Page() {
  ...
  const handleSearchSubmit = async (e) => {
    ...
    const query = generateSparqlQuery(searchQuery);
        ...
        {results.length > 0 ? (
          results.map((result, index) => (
            <div key={index} style={styles.resultItem}>
              <Link href={`/profil/${encodeURIComponent(result.dish.value.split('/').pop())}`}>
                <a style={styles.link}>
